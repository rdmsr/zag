//! Lock-free multi-producer multi-consumer ring buffer designed
//! for kernel logging.
//!
//! # Overview
//!
...
//! there is no support for pr_cont or the "committed" state.
//! See linux's printk_ringbuffer.c for more details.
//!
//! The ring buffer is composed of three parallel arrays:
//!
//!   - `descs`:  tracks the state and data location of each record
//!   - `data`:  byte storage for the actual messages
//!   - `infos`: metadata: sequence number, timestamp, and length per record
//!
//! `infos` is indexed by descriptor ID, so it doesn't *really* count as a ring.
//! `head_id` is the ID of the newest descriptor, the last slot claimed by a
//!  writer.
//! `tail_id` is the ID of the oldest descriptor, the oldest slot which is the
//! next to be evicted.
...
//! when this reaches the maximum descriptor count (`desc_count`),
//! the ring is full and `tail_id` must be advanced.
//! `head_lpos` and `tail_lpos` are the logical positions of the next data block
//!  to be written and the oldest data block, respectively.
//!
//! # Descriptor states
//!
...
//!   `Miss`       Pseudo-state: the slot has been reused in a newer generation.
//!                Returned when the ID embedded in a descriptor state does not
//!                match the expected ID, indicating the record is gone.
//!
...
//! `usize`, allowing both to be read and updated atomically via CAS
//! (a CAS is used to prevent ABA where the generation could overflow):
//!
...
// Thus, 1 (an odd number) is guaranteed to never collide with a real data
// block position.
...
pub fn RingBuffer(data_size_bits: usize, avg_msg_bits: usize) type {
    return struct {
        ...
        fn check_size(size: usize) bool {
            // If the size is larger than half of the data buffer,
            // it can never fit in the ring buffer.
        ...
        fn read_desc_finalized(
            ...
            if (res.state == .Miss or res.state == .Reserved or
                (res.seq != seq))
            {
        ...
        fn free_data(self: *Self, begin: usize, end: usize) ?usize {
            ...
            while (need_more_space(cur_begin, end)) {
                ...
                // Loading the ID here is purposely racey because
                // we check if the descriptor is valid anyway.
                const id: Id = @truncate(
                    @atomicLoad(
                        usize,
                        @as(*usize, @ptrCast(&blk.id)),
                        .monotonic,
                    ),
                );
                const res = self.read_desc(id);
                ...
                switch (res.state) {
                    ...
                    .Published => {
                        if (desc.blk_pos.begin != cur_begin) {
                            // The descriptor does not point back to
                            // the data block, we must've lost a race.
                            return null;
                        }
                        self.free_desc(id);
                    },
        ...
        fn advance_tail(self: *Self, tail_id: Id) !void {
            ...
            switch (res.state) {
                ...
                .Reserved => {
                    // The descriptor is still reserved,
                    // we can't do anything but fail the reservation.
                    return error.ReservationFailed;
                },
                .Miss => {
                ...
                .Free => {
                    // The descriptor is already free, we can just advance the
                    // tail.
                },
            }
            ...
            // Read the next descriptor after tail_id, because the tail must
            // always be in a finalized or free state.
            ...
            if (next_res.state == .Published or next_res.state == .Free) {
                ...
                // One possibility is that the tail was pushed by another CPU,
        ...
        fn reserve_desc(self: *Self) !Id {
            ...
            while (true) {
                ...
                if (prev_id == tail_id) {
                    // Wrapped back to the tail, need to make space by
                    // advancing it.
                    try self.advance_tail(prev_id);
                }

                // Ordering: acq_rel to claim head and publish,
                // acquire on failure to retry.
                if (self.head_id.cmpxchgWeak(
                    head_id,
                    new_id,
                    .acq_rel,
                    .acquire,
            ...
            // Ordering: monotonic is safe here because acq_rel on head_id
            // published our claim.
            const prev_state = desc.state.load(.monotonic);
            const raw: usize = @bitCast(prev_state);

            if (raw != 0 and
                get_desc_state(prev_id, prev_state) != .Free)
            {
                return error.ReservationFailed;
            }

            // Ordering: release ensures all setup is published before making
            // the descriptor reserved.
            if (desc.state.cmpxchgStrong(
                prev_state,
                .{
                    .state = .Reserved,
                    .id = new_id,
        ...
        // The -% 1 ensures a block that fits exactly at the end does not count
        // as wrapping.
        ...
        fn get_next_lpos(lpos: usize, size: usize) usize {
            ...
            // If it would cross the boundary, we advance the `lpos` to the next
            // generation boundary so the data can sit contiguously at
            // physical offset 0.
        ...
        pub fn init() Self {
            ...
            // in slot 0, giving the first real record sequence number 0.
            self.descs[desc_count - 1].state.store(.{
                .state = .Free,
                .id = dummy_id,
            }, .monotonic);

            self.descs[desc_count - 1].blk_pos = .{
                .begin = lpos_no_data,
                .end = lpos_no_data,
            };
            ...
            // Make it -(desc_count) so that when the slot is first reserved and
            // the sequence is advanced by desc_count, the first record gets
            // sequence number 0.
        ...
        pub fn reserve(self: *Self, size: usize) ReserveError!Reservation {
            ...
            reservation.buf = self.alloc_data(
                size,
                id,
                &desc.blk_pos,
            ) catch |e| {
                // On failure, publish the descriptor as-is to clean up
                // the reservation.
