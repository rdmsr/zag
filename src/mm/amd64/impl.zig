...
pub fn init_kernel() void {
    mi.kernel_space.pmap.root_pa = mi.phys.early_alloc();
    const table_ptr: [*]Pte = @ptrFromInt(mm.p2v(mi.kernel_space.pmap.root_pa));
    @memset(table_ptr[0..512], Pte.zero());

    for (256..512) |i| {
        table_ptr[i] = make_table_pte(mi.phys.early_alloc());
    }
}
