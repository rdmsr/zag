...
fn level_shift(level: usize) u6 {
    return mmp.impl.levels[level].shift;
}
...
fn index_for_level(va: usize, level: usize) usize {
    return (va >> level_shift(level)) & mmp.impl.levels[level].mask;
}
...
pub const PMap = struct {
    ...
    fn is_leaf_level(level: usize) bool {
        return mmp.impl.levels[level].leaf;
    }
        ...
        for (0..npages) |_| {
            ...
            blk: while (true) {
                ...
                if (level == 0) {
                    ...
                    c.tables[level][idx] = std.mem.zeroes(mmp.impl.Pte);
                    break;
                }
    ...
    pub const Cursor = struct {
        ...
        tables: [num_levels][*]mmp.impl.Pte,
        ...
        fn walk_down(
            ...
            while (current_level > target_level) {
                ...
                var pte = mmp.impl.Pte.load(&self.tables[current_level][idx]);
                ...
                if (!pte.present) {
                    ...
                    const new_table_pa = mmp.phys.alloc();

                    const table_ptr: [*]mmp.impl.Pte = @ptrFromInt(
                        mm.p2v(new_table_pa),
                    );
                    @memset(
                        table_ptr[0..entries_per_table],
                        std.mem.zeroes(mmp.impl.Pte),
                    );

                    pte = mmp.impl.make_table_pte(new_table_pa);
        ...
        pub fn map_range(
            ...
            while (remain > 0) {
                ...
                table[index_for_level(self.va, target_level)] =
                    mmp.impl.make_leaf_pte(current_pa, flags, target_level);
