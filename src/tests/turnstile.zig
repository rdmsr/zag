...
fn spawn(prio: u8, entry: *const fn (?*anyopaque) void, arg: ?*anyopaque) *ps.Thread {
    const t = ps.thread.create_kernel(@enumFromInt(prio), .{ .func = entry, .arg = arg }, true) catch @panic("oom");
...
pub fn start(_: ?*anyopaque) void {
    ...
    for (0..nstress) |i| {
        ...
        const t = ps.thread.create_kernel(@enumFromInt(prio), .{ .func = &stress, .arg = @ptrFromInt(i + 1) }, true) catch @panic("oom");
        stress_tds[i] = t;
        ke.sched.enqueue(&t.kern);
    }
